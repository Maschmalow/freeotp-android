...
public class TokenPersistence {
    ...
    public  void convert(String pwd, Uri uri) {
        try {
            List<RestoredData> tokenList = restore(pwd);
            ...
            for(RestoredData token : tokenList) {
                output.println(token.token.toUri(token.key).toString());
            }
            fileOutputStream.close();
            pfd.close();

        } catch (IOException | BadPasswordException | GeneralSecurityException | JSONException e) {
